...
class Settings(BaseSettings):
    ...
    project_path: Path = Field(
        default_factory=Path.cwd, description="Root path for KiCad projects"
    )

    board_file: Optional[Path] = Field(
        default=None, description="Path to specific board file to load on startup"
    )

    # KiCad integration settings
    kicad_version: str = Field(default="9.0", description="Expected KiCad version")
    ...
    use_flatpak: bool = Field(
        default=False, description="Whether to use KiCad Flatpak installation"
    )

    # Server settings
    server_name: str = Field(
        default="kicad-mcp-server", description="MCP server name identifier"
    )
    ...
    operation_timeout_seconds: int = Field(
        default=300, ge=1, le=3600, description="Timeout for operations in seconds"
    )
    ...
    create_timestamped_exports: bool = Field(
        default=True, description="Add timestamps to export directory names"
    )

    zip_exports: bool = Field(
        default=True, description="Create ZIP archives for fabrication packages"
    )
    ...
    log_rotation_mb: int = Field(
        default=10, ge=1, le=1000, description="Log file size before rotation (MB)"
    )

    log_retention_days: int = Field(
        default=7, ge=1, le=365, description="Number of days to keep old log files"
    )

    # Visual verification settings
    enable_visual_verification: bool = Field(
        default=True, description="Enable screenshot capture for visual feedback"
    )

    screenshot_width: int = Field(
        default=1600, ge=800, le=4000, description="Screenshot width in pixels"
    )

    screenshot_height: int = Field(
        default=1200, ge=600, le=3000, description="Screenshot height in pixels"
    )

    screenshot_quality: int = Field(
        default=85, ge=1, le=100, description="Screenshot JPEG quality (1-100)"
    )
    ...
    enable_undo: bool = Field(
        default=True, description="Enable undo/redo functionality"
    )

    persist_history: bool = Field(
        default=False, description="Persist operation history to disk"
    )
    ...
    allow_external_commands: bool = Field(
        default=False, description="Allow execution of external commands"
    )
    ...
    enable_mock_mode: bool = Field(
        default=False, description="Force mock mode even if KiCad is available"
    )

    # Performance settings
    enable_caching: bool = Field(
        default=True, description="Enable caching of board information"
    )
